...
namespace space1
{
    ...
    public class Topo //拓扑类
    {
        ...
        public int printDistance()
        {
            int max = 0;

            for (int i = 0; i < size; i++)
            {
                if (wordChainList[i].isChain())
                {
                    if (wordChainList[i].getWeight() > max)
                    {
                        max = wordChainList[i].getWeight();
                    }
                }
            ...
            for (int i=0; i<size; i++)
            {
                if(wordChainList[i].isChain() && wordChainList[i].getWeight() == max )
                {
                    wordChainList[i].printChain();
                }
            }
